...
enum class Dirty_flag {
    none = 0, buffer, offset, image, sampler
};

//----------------------------------------------------------------------------------------------------------------------

inline auto& operator|=(uint32_t& flags, Pipeline_stage stage)
{
    return flags |= static_cast<uint32_t>(stage);
}

//----------------------------------------------------------------------------------------------------------------------

inline auto operator&(uint8_t flags, Pipeline_stage stage)
{
    return flags & static_cast<uint8_t>(stage);
}
...
void Mtl_render_encoder::bind_arg_table_()
{
    ...
    for (auto& [index, stages] : reflection.buffers) {
        ...
        if (!(arg_buffer.buffer_stages & Pipeline_stage::vertex_shader)) {
        ...
        else if (!(arg_buffer.offset_stages & Pipeline_stage::vertex_shader)) {
            [render_command_encoder_ setVertexBufferOffset:arg_buffer.offset
                                                   atIndex:index];
            arg_buffer.offset_stages |= Pipeline_stage::vertex_shader;
        }

        if (!(arg_buffer.buffer_stages & Pipeline_stage::fragment_shader)) {
        ...
        else if (!(arg_buffer.offset_stages & Pipeline_stage::fragment_shader)) {
    ...
    for (auto& [index, stages] : reflection.textures) {
        ...
        if (!(arg_texture.image_stages & Pipeline_stage::fragment_shader)) {
        ...
        if (!(arg_texture.sampler_stages & Pipeline_stage::fragment_shader)) {
